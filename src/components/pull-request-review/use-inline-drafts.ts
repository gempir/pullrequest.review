...
const INLINE_DRAFT_PREFIX = makeVersionedStorageKey(INLINE_DRAFT_PREFIX_BASE, 2);

const INLINE_ACTIVE_PREFIX_BASE = "pr_review_inline_comment_active";
const INLINE_ACTIVE_PREFIX = makeVersionedStorageKey(INLINE_ACTIVE_PREFIX_BASE, 2);
...
export function inlineActiveDraftStorageKey(workspace: string, repo: string, pullRequestId: string) {
    return `${INLINE_ACTIVE_PREFIX}:${workspace}/${repo}/${pullRequestId}`;
}

export function parseInlineDraftStorageKey(key: string, workspace: string, repo: string, pullRequestId: string): InlineDraftLocation | null {
