...
import { approvePullRequest, mergePullRequest, requestChangesOnPullRequest } from "@/lib/git-host/service";
...
export function useReviewActions({ prRef, queryKey }: UseReviewActionsProps) {
    ...
    const merge = useMutation({
        mutationFn: async (payload: { message?: string; mergeStrategy?: string; closeSourceBranch?: boolean }) => {
            if (!prRef) throw new Error("Pull request reference is missing");
