...
function normalizeBundleRecord(
  value: unknown,
): PullRequestBundleRecord | undefined {
  ...
  return {
    ...
    buildStatuses: Array.isArray(normalized.buildStatuses)
      ? normalized.buildStatuses
      : [],
  };
}
...
export function useReviewQuery({
  ...
  const bundleId = useMemo(
    () => `${host}:${workspace}/${repo}/${pullRequestId}`,
  ...
  const bundleQuery = useLiveQuery(
    (q) => {
      ...
      return q
        .from({ bundle: bundleStore.collection })
  ...
  const queryData = useMemo(
    () =>
      (bundleQuery.data ?? [])
        .map(normalizeBundleRecord)
        .find((record) => record?.id === bundleId),
    [bundleId, bundleQuery.data],
  );
  const collectionError = bundleStore?.utils.lastError;
  const queryError = collectionError;
  const queryIsLoading = canLoadPullRequest
    ? bundleQuery.isLoading && !queryData
    : false;
  const queryIsFetching = canLoadPullRequest
    ? Boolean(bundleStore?.utils.isFetching)
    : false;
  const refetchQuery = useCallback(
    () =>
      bundleStore?.utils.refetch({ throwOnError: false }) ?? Promise.resolve(),
    [bundleStore],
  );
  ...
  const hasPendingBuildStatuses =
    queryData?.buildStatuses?.some((status) => status.state === "pending") ??
    false;
