...
const buttonVariants = cva(
    "inline-flex cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-sm font-medium transition-[color,background-color,border-color,box-shadow,transform] duration-150 ease-out disabled:pointer-events-none disabled:opacity-50 disabled:cursor-not-allowed [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background",
    {
        variants: {
            variant: {
                default:
                ...
                destructive:
                    "bg-destructive-solid text-destructive-foreground border border-destructive-solid shadow-sm hover:bg-destructive-solid/90 active:bg-destructive-solid",
...
function Button({
    ...
    return (
        <Comp
            ...
            data-size={size}
            className={cn(!staticMotion && "active:scale-[0.96] motion-reduce:active:scale-100", buttonVariants({ variant, size, className }))}
            {...props}
        />
    );
}
