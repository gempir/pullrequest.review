...
function parseStoredCredentials(rawValue: string | null): BitbucketSessionCookie | null {
...
export function setBitbucketCredentials(email: string, apiToken: string) {
    writeStorageValue(BITBUCKET_AUTH_KEY, JSON.stringify({ email, apiToken } satisfies BitbucketSessionCookie));
}
