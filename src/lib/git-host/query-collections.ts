...
let repoPullRequestCollection: Collection<
  PersistedRepoPullRequestRecord,
  string
> | null = null;
let pullRequestBundleCollection: Collection<
  PersistedPullRequestBundleRecord,
  string
> | null = null;

const repositoryScopedCollections = new Map<
  GitHost,
  ScopedCollection<RepositoryRecord>
>();
const repoPullRequestScopedCollections = new Map<
  string,
  ScopedCollection<PersistedRepoPullRequestRecord>
>();
const pullRequestBundleScopedCollections = new Map<
  string,
  ScopedCollection<PersistedPullRequestBundleRecord>
>();

function createCollectionUtils(
  refetch: (opts?: { throwOnError?: boolean }) => Promise<void>,
): CollectionUtils {
...
function normalizeRepoRef(repo: RepoRef): RepoRef {
  ...
  const displayName =
    typeof repo.displayName === "string" && repo.displayName.trim().length > 0
...
function normalizeReposByHost(reposByHost: ReposByHost): ReposByHost {
  return {
    bitbucket: [...(reposByHost.bitbucket ?? [])]
      .map(normalizeRepoRef)
      .sort((a, b) => a.fullName.localeCompare(b.fullName)),
    github: [...(reposByHost.github ?? [])]
      .map(normalizeRepoRef)
      .sort((a, b) => a.fullName.localeCompare(b.fullName)),
  };
}
...
function serializePullRequestBundle(
...
function serializeRepoPullRequestRecord(
  repo: RepoRef,
...
export function isValidPullRequestBundleRecord(
  value: unknown,
): value is PullRequestBundleRecord {
  if (!isRecord(value)) return false;
...
function isValidPullRequestSummary(
...
function normalizePullRequestSummary(
  ...
  const title =
    typeof pullRequest.title === "string" && pullRequest.title.trim().length > 0
      ...
      : `#${pullRequest.id}`;
  const state =
    typeof pullRequest.state === "string" && pullRequest.state.trim().length > 0
      ? pullRequest.state
      : "OPEN";
...
export function ensureGitHostDataReady() {
  if (!hostDataReadyPromise) {
    hostDataReadyPromise = initRxdbCollections().catch((error) => {
      console.error(
        "Failed to initialize RxDB host-data collections, falling back to in-memory collections.",
        error,
      );
      ensureCollectionsInitialized();
    });
...
export function getRepoPullRequestCollection(data: {
  hosts: GitHost[];
  ...
  const utils = createCollectionUtils(async (opts) => {
    ...
    for (const host of normalizedHosts) {
      for (const repo of normalizedReposByHost[host] ?? []) {
        const normalizedRepo = normalizeRepoRef(repo);
        selectedRepoKeys.add(
          `${normalizedRepo.host}:${normalizedRepo.fullName}`,
        );
      }
    }
    ...
    try {
      ...
      const nextRecords = response.flatMap(({ repo, pullRequests }) => {
        if (!isValidRepoRef(repo))
          return [] as PersistedRepoPullRequestRecord[];
        return pullRequests
          .filter((pullRequest) => isValidPullRequestSummary(pullRequest))
          .map((pullRequest) =>
            serializeRepoPullRequestRecord(
              repo,
              normalizePullRequestSummary(pullRequest),
            ),
          );
      });
...
export function getRepositoryCollection(host: GitHost) {
  ...
  const utils = createCollectionUtils(async (opts) => {
    ...
    try {
      ...
      const nextRecords = repositories
        .map(normalizeRepoRef)
        .map((repository) => ({
          ...repository,
