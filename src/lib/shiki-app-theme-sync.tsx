...
function buildPalette(theme: ShikiThemeLike) {
  ...
  const background =
    ...
    ]) ?? (lightTheme ? "#f7f7f8" : "#0a0a0a");
  const foreground =
    pickColor(colors, ["editor.foreground", "foreground"]) ??
    (lightTheme ? "#141414" : "#e0e0e0");
  const selection =
    pickColor(colors, [
      "editor.selectionBackground",
      "editor.lineHighlightBackground",
      "focusBorder",
    ]) ?? mixColors(background, foreground, 0.2);
  ...
  const sidebar = mixColors(
    background,
    foreground,
    darkBackground ? 0.05 : 0.03,
  );
  const sidebarAccent = mixColors(
    sidebar,
    foreground,
    darkBackground ? 0.14 : 0.1,
  );
