...
function normalizePullRequestRecord(record: unknown): {
    ...
    const fullName =
        typeof repoSource.fullName === "string" && repoSource.fullName.trim().length ? repoSource.fullName.trim() : `${workspace}/${repositorySlug}`;
    const displayName = typeof repoSource.displayName === "string" && repoSource.displayName.trim().length ? repoSource.displayName.trim() : repositorySlug;
    const title = typeof pullRequestSource.title === "string" && pullRequestSource.title.trim().length ? pullRequestSource.title.trim() : `#${pullRequestId}`;

    return {
        repoKey: typeof value.repoKey === "string" && value.repoKey.trim().length ? value.repoKey : `${value.host}:${fullName}`,
        host: value.host,
        repo: {
            host: value.host,
            workspace,
...
function useLandingPageView({ initialHost, initialDiffPanel = "pull-requests" }: { initialHost?: GitHost; initialDiffPanel?: DiffPanel } = {}) {
    ...
    const groupedPullRequests = useMemo(() => {
        ...
        const groupedByRepo = new Map<string, { host: GitHost; repo: RepoRef; pullRequests: PullRequestSummary[] }>();
    ...
    const selectedRepoCount = reposByHost.bitbucket.length + reposByHost.github.length;
            ...
            <section className="flex-1 min-w-0 min-h-0 flex flex-col">
                ...
                <main data-component="diff-view" className={cn("flex-1 min-h-0 overflow-y-auto", showSettingsPanel ? "p-0" : "p-4")}>
                    {showSettingsPanel ? (
                        ...
                        <div className="max-w-3xl space-y-4">
                            {authByHost[activeHost] ? (
                                <>
                                    ...
                                    <div className="app-card-muted p-3 space-y-2">
                        ...
                        <div className="app-card p-8 text-center space-y-3 max-w-2xl">
                        ...
                        <div className="app-card p-4 text-destructive text-[13px] max-w-2xl bg-destructive/10">
                            ...
                            <div>
                                {groupedPullRequests
                                    ...
                                    .map(({ host, repo, pullRequests }) => (
                                        <div key={`${host}:${repo.fullName}`} className="border-b border-subtle-border p-3 last:border-b-0">
                                            <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-medium mb-2 flex items-center gap-2">
                                                <span className="rounded-[2px] border border-subtle-border bg-muted px-1.5 py-0.5">{getHostLabel(host)}</span>
                                            ...
                                            <div className="space-y-1">
                                                {pullRequests.map((pr) => (
                                                    <button
                                                        ...
                                                        className="app-list-row w-full text-left px-3 py-2.5 text-[13px]"
                                                        onClick={() => {
                                                            if (repo.host === "github") {
                                                                navigate({
                                                                    to: "/$workspace/$repo/pull/$pullRequestId",
