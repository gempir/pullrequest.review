...
describe("commit range diff providers", () => {
    ...
    test("Bitbucket revspec endpoints map diff and diffstat", async () => {
        writeBitbucketAuthCredential({ email: "user@example.com", apiToken: "token" });
        const calls = [];
        globalThis.fetch = async (input) => {
            const url = String(input);
            calls.push(url);
            if (url.includes("/diffstat/aaa11111..bbb22222")) {
                return Response.json({
                    values: [
                        {
                            status: "modified",
            ...
            if (url.includes("/diff/aaa11111..bbb22222")) {
                return new Response("diff --git a/src/a.ts b/src/a.ts\n", { status: 200 });
            }
            return new Response("Not found", { status: 404 });
        };
        ...
        expect(calls.some((url) => url.includes("/diff/aaa11111..bbb22222"))).toBeTrue();
        expect(calls.some((url) => url.includes("/diffstat/aaa11111..bbb22222?pagelen=100"))).toBeTrue();
        expect(result.diffstat).toEqual([
            {
                status: "modified",
                new: { path: "src/a.ts" },
